// Code generated by goa, DO NOT EDIT.
//
// ingest HTTP server encoders and decoders
//
// Command:
// $ goa gen github.com/artefactual-sdps/enduro/internal/api/design -o
// internal/api

package server

import (
	"context"
	"errors"
	"io"
	"net/http"
	"strconv"
	"strings"

	ingest "github.com/artefactual-sdps/enduro/internal/api/gen/ingest"
	ingestviews "github.com/artefactual-sdps/enduro/internal/api/gen/ingest/views"
	goahttp "goa.design/goa/v3/http"
	goa "goa.design/goa/v3/pkg"
)

// EncodeMonitorRequestResponse returns an encoder for responses returned by
// the ingest monitor_request endpoint.
func EncodeMonitorRequestResponse(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder) func(context.Context, http.ResponseWriter, any) error {
	return func(ctx context.Context, w http.ResponseWriter, v any) error {
		res, _ := v.(*ingest.MonitorRequestResult)
		if res.Ticket != nil {
			ticket := *res.Ticket
			http.SetCookie(w, &http.Cookie{
				Name:     "enduro-ingest-ws-ticket",
				Value:    ticket,
				MaxAge:   5,
				Secure:   true,
				HttpOnly: true,
			})
		}
		w.WriteHeader(http.StatusOK)
		return nil
	}
}

// DecodeMonitorRequestRequest returns a decoder for requests sent to the
// ingest monitor_request endpoint.
func DecodeMonitorRequestRequest(mux goahttp.Muxer, decoder func(*http.Request) goahttp.Decoder) func(*http.Request) (*ingest.MonitorRequestPayload, error) {
	return func(r *http.Request) (*ingest.MonitorRequestPayload, error) {
		var payload *ingest.MonitorRequestPayload
		var (
			token *string
		)
		tokenRaw := r.Header.Get("Authorization")
		if tokenRaw != "" {
			token = &tokenRaw
		}
		payload = NewMonitorRequestPayload(token)
		if payload.Token != nil {
			if strings.Contains(*payload.Token, " ") {
				// Remove authorization scheme prefix (e.g. "Bearer")
				cred := strings.SplitN(*payload.Token, " ", 2)[1]
				payload.Token = &cred
			}
		}

		return payload, nil
	}
}

// EncodeMonitorRequestError returns an encoder for errors returned by the
// monitor_request ingest endpoint.
func EncodeMonitorRequestError(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder, formatter func(ctx context.Context, err error) goahttp.Statuser) func(context.Context, http.ResponseWriter, error) error {
	encodeError := goahttp.ErrorEncoder(encoder, formatter)
	return func(ctx context.Context, w http.ResponseWriter, v error) error {
		var en goa.GoaErrorNamer
		if !errors.As(v, &en) {
			return encodeError(ctx, w, v)
		}
		switch en.GoaErrorName() {
		case "internal_error":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewMonitorRequestInternalErrorResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusInternalServerError)
			return enc.Encode(body)
		case "forbidden":
			var res ingest.Forbidden
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusForbidden)
			return enc.Encode(body)
		case "unauthorized":
			var res ingest.Unauthorized
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusUnauthorized)
			return enc.Encode(body)
		default:
			return encodeError(ctx, w, v)
		}
	}
}

// DecodeMonitorRequest returns a decoder for requests sent to the ingest
// monitor endpoint.
func DecodeMonitorRequest(mux goahttp.Muxer, decoder func(*http.Request) goahttp.Decoder) func(*http.Request) (*ingest.MonitorPayload, error) {
	return func(r *http.Request) (*ingest.MonitorPayload, error) {
		var payload *ingest.MonitorPayload
		var (
			ticket *string
			c      *http.Cookie
		)
		c, _ = r.Cookie("enduro-ingest-ws-ticket")
		var ticketRaw string
		if c != nil {
			ticketRaw = c.Value
		}
		if ticketRaw != "" {
			ticket = &ticketRaw
		}
		payload = NewMonitorPayload(ticket)

		return payload, nil
	}
}

// EncodeMonitorError returns an encoder for errors returned by the monitor
// ingest endpoint.
func EncodeMonitorError(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder, formatter func(ctx context.Context, err error) goahttp.Statuser) func(context.Context, http.ResponseWriter, error) error {
	encodeError := goahttp.ErrorEncoder(encoder, formatter)
	return func(ctx context.Context, w http.ResponseWriter, v error) error {
		var en goa.GoaErrorNamer
		if !errors.As(v, &en) {
			return encodeError(ctx, w, v)
		}
		switch en.GoaErrorName() {
		case "internal_error":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewMonitorInternalErrorResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusInternalServerError)
			return enc.Encode(body)
		case "forbidden":
			var res ingest.Forbidden
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusForbidden)
			return enc.Encode(body)
		case "unauthorized":
			var res ingest.Unauthorized
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusUnauthorized)
			return enc.Encode(body)
		default:
			return encodeError(ctx, w, v)
		}
	}
}

// EncodeListSipsResponse returns an encoder for responses returned by the
// ingest list_sips endpoint.
func EncodeListSipsResponse(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder) func(context.Context, http.ResponseWriter, any) error {
	return func(ctx context.Context, w http.ResponseWriter, v any) error {
		res := v.(*ingestviews.SIPs)
		enc := encoder(ctx, w)
		body := NewListSipsResponseBody(res.Projected)
		w.WriteHeader(http.StatusOK)
		return enc.Encode(body)
	}
}

// DecodeListSipsRequest returns a decoder for requests sent to the ingest
// list_sips endpoint.
func DecodeListSipsRequest(mux goahttp.Muxer, decoder func(*http.Request) goahttp.Decoder) func(*http.Request) (*ingest.ListSipsPayload, error) {
	return func(r *http.Request) (*ingest.ListSipsPayload, error) {
		var payload *ingest.ListSipsPayload
		var (
			name                *string
			aipUUID             *string
			earliestCreatedTime *string
			latestCreatedTime   *string
			status              *string
			uploaderUUID        *string
			batchUUID           *string
			limit               *int
			offset              *int
			token               *string
			err                 error
		)
		qp := r.URL.Query()
		nameRaw := qp.Get("name")
		if nameRaw != "" {
			name = &nameRaw
		}
		aipUUIDRaw := qp.Get("aip_uuid")
		if aipUUIDRaw != "" {
			aipUUID = &aipUUIDRaw
		}
		if aipUUID != nil {
			err = goa.MergeErrors(err, goa.ValidateFormat("aip_uuid", *aipUUID, goa.FormatUUID))
		}
		earliestCreatedTimeRaw := qp.Get("earliest_created_time")
		if earliestCreatedTimeRaw != "" {
			earliestCreatedTime = &earliestCreatedTimeRaw
		}
		if earliestCreatedTime != nil {
			err = goa.MergeErrors(err, goa.ValidateFormat("earliest_created_time", *earliestCreatedTime, goa.FormatDateTime))
		}
		latestCreatedTimeRaw := qp.Get("latest_created_time")
		if latestCreatedTimeRaw != "" {
			latestCreatedTime = &latestCreatedTimeRaw
		}
		if latestCreatedTime != nil {
			err = goa.MergeErrors(err, goa.ValidateFormat("latest_created_time", *latestCreatedTime, goa.FormatDateTime))
		}
		statusRaw := qp.Get("status")
		if statusRaw != "" {
			status = &statusRaw
		}
		if status != nil {
			if !(*status == "error" || *status == "failed" || *status == "queued" || *status == "processing" || *status == "pending" || *status == "ingested" || *status == "validated" || *status == "canceled") {
				err = goa.MergeErrors(err, goa.InvalidEnumValueError("status", *status, []any{"error", "failed", "queued", "processing", "pending", "ingested", "validated", "canceled"}))
			}
		}
		uploaderUUIDRaw := qp.Get("uploader_uuid")
		if uploaderUUIDRaw != "" {
			uploaderUUID = &uploaderUUIDRaw
		}
		if uploaderUUID != nil {
			err = goa.MergeErrors(err, goa.ValidateFormat("uploader_uuid", *uploaderUUID, goa.FormatUUID))
		}
		batchUUIDRaw := qp.Get("batch_uuid")
		if batchUUIDRaw != "" {
			batchUUID = &batchUUIDRaw
		}
		if batchUUID != nil {
			err = goa.MergeErrors(err, goa.ValidateFormat("batch_uuid", *batchUUID, goa.FormatUUID))
		}
		{
			limitRaw := qp.Get("limit")
			if limitRaw != "" {
				v, err2 := strconv.ParseInt(limitRaw, 10, strconv.IntSize)
				if err2 != nil {
					err = goa.MergeErrors(err, goa.InvalidFieldTypeError("limit", limitRaw, "integer"))
				}
				pv := int(v)
				limit = &pv
			}
		}
		{
			offsetRaw := qp.Get("offset")
			if offsetRaw != "" {
				v, err2 := strconv.ParseInt(offsetRaw, 10, strconv.IntSize)
				if err2 != nil {
					err = goa.MergeErrors(err, goa.InvalidFieldTypeError("offset", offsetRaw, "integer"))
				}
				pv := int(v)
				offset = &pv
			}
		}
		tokenRaw := r.Header.Get("Authorization")
		if tokenRaw != "" {
			token = &tokenRaw
		}
		if err != nil {
			return payload, err
		}
		payload = NewListSipsPayload(name, aipUUID, earliestCreatedTime, latestCreatedTime, status, uploaderUUID, batchUUID, limit, offset, token)
		if payload.Token != nil {
			if strings.Contains(*payload.Token, " ") {
				// Remove authorization scheme prefix (e.g. "Bearer")
				cred := strings.SplitN(*payload.Token, " ", 2)[1]
				payload.Token = &cred
			}
		}

		return payload, nil
	}
}

// EncodeListSipsError returns an encoder for errors returned by the list_sips
// ingest endpoint.
func EncodeListSipsError(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder, formatter func(ctx context.Context, err error) goahttp.Statuser) func(context.Context, http.ResponseWriter, error) error {
	encodeError := goahttp.ErrorEncoder(encoder, formatter)
	return func(ctx context.Context, w http.ResponseWriter, v error) error {
		var en goa.GoaErrorNamer
		if !errors.As(v, &en) {
			return encodeError(ctx, w, v)
		}
		switch en.GoaErrorName() {
		case "not_valid":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewListSipsNotValidResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusBadRequest)
			return enc.Encode(body)
		case "forbidden":
			var res ingest.Forbidden
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusForbidden)
			return enc.Encode(body)
		case "unauthorized":
			var res ingest.Unauthorized
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusUnauthorized)
			return enc.Encode(body)
		default:
			return encodeError(ctx, w, v)
		}
	}
}

// EncodeShowSipResponse returns an encoder for responses returned by the
// ingest show_sip endpoint.
func EncodeShowSipResponse(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder) func(context.Context, http.ResponseWriter, any) error {
	return func(ctx context.Context, w http.ResponseWriter, v any) error {
		res := v.(*ingestviews.SIP)
		enc := encoder(ctx, w)
		body := NewShowSipResponseBody(res.Projected)
		w.WriteHeader(http.StatusOK)
		return enc.Encode(body)
	}
}

// DecodeShowSipRequest returns a decoder for requests sent to the ingest
// show_sip endpoint.
func DecodeShowSipRequest(mux goahttp.Muxer, decoder func(*http.Request) goahttp.Decoder) func(*http.Request) (*ingest.ShowSipPayload, error) {
	return func(r *http.Request) (*ingest.ShowSipPayload, error) {
		var payload *ingest.ShowSipPayload
		var (
			uuid  string
			token *string
			err   error

			params = mux.Vars(r)
		)
		uuid = params["uuid"]
		err = goa.MergeErrors(err, goa.ValidateFormat("uuid", uuid, goa.FormatUUID))
		tokenRaw := r.Header.Get("Authorization")
		if tokenRaw != "" {
			token = &tokenRaw
		}
		if err != nil {
			return payload, err
		}
		payload = NewShowSipPayload(uuid, token)
		if payload.Token != nil {
			if strings.Contains(*payload.Token, " ") {
				// Remove authorization scheme prefix (e.g. "Bearer")
				cred := strings.SplitN(*payload.Token, " ", 2)[1]
				payload.Token = &cred
			}
		}

		return payload, nil
	}
}

// EncodeShowSipError returns an encoder for errors returned by the show_sip
// ingest endpoint.
func EncodeShowSipError(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder, formatter func(ctx context.Context, err error) goahttp.Statuser) func(context.Context, http.ResponseWriter, error) error {
	encodeError := goahttp.ErrorEncoder(encoder, formatter)
	return func(ctx context.Context, w http.ResponseWriter, v error) error {
		var en goa.GoaErrorNamer
		if !errors.As(v, &en) {
			return encodeError(ctx, w, v)
		}
		switch en.GoaErrorName() {
		case "not_available":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewShowSipNotAvailableResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusConflict)
			return enc.Encode(body)
		case "not_found":
			var res *ingest.SIPNotFound
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewShowSipNotFoundResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusNotFound)
			return enc.Encode(body)
		case "forbidden":
			var res ingest.Forbidden
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusForbidden)
			return enc.Encode(body)
		case "unauthorized":
			var res ingest.Unauthorized
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusUnauthorized)
			return enc.Encode(body)
		default:
			return encodeError(ctx, w, v)
		}
	}
}

// EncodeListSipWorkflowsResponse returns an encoder for responses returned by
// the ingest list_sip_workflows endpoint.
func EncodeListSipWorkflowsResponse(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder) func(context.Context, http.ResponseWriter, any) error {
	return func(ctx context.Context, w http.ResponseWriter, v any) error {
		res := v.(*ingestviews.SIPWorkflows)
		enc := encoder(ctx, w)
		body := NewListSipWorkflowsResponseBody(res.Projected)
		w.WriteHeader(http.StatusOK)
		return enc.Encode(body)
	}
}

// DecodeListSipWorkflowsRequest returns a decoder for requests sent to the
// ingest list_sip_workflows endpoint.
func DecodeListSipWorkflowsRequest(mux goahttp.Muxer, decoder func(*http.Request) goahttp.Decoder) func(*http.Request) (*ingest.ListSipWorkflowsPayload, error) {
	return func(r *http.Request) (*ingest.ListSipWorkflowsPayload, error) {
		var payload *ingest.ListSipWorkflowsPayload
		var (
			uuid  string
			token *string
			err   error

			params = mux.Vars(r)
		)
		uuid = params["uuid"]
		err = goa.MergeErrors(err, goa.ValidateFormat("uuid", uuid, goa.FormatUUID))
		tokenRaw := r.Header.Get("Authorization")
		if tokenRaw != "" {
			token = &tokenRaw
		}
		if err != nil {
			return payload, err
		}
		payload = NewListSipWorkflowsPayload(uuid, token)
		if payload.Token != nil {
			if strings.Contains(*payload.Token, " ") {
				// Remove authorization scheme prefix (e.g. "Bearer")
				cred := strings.SplitN(*payload.Token, " ", 2)[1]
				payload.Token = &cred
			}
		}

		return payload, nil
	}
}

// EncodeListSipWorkflowsError returns an encoder for errors returned by the
// list_sip_workflows ingest endpoint.
func EncodeListSipWorkflowsError(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder, formatter func(ctx context.Context, err error) goahttp.Statuser) func(context.Context, http.ResponseWriter, error) error {
	encodeError := goahttp.ErrorEncoder(encoder, formatter)
	return func(ctx context.Context, w http.ResponseWriter, v error) error {
		var en goa.GoaErrorNamer
		if !errors.As(v, &en) {
			return encodeError(ctx, w, v)
		}
		switch en.GoaErrorName() {
		case "not_found":
			var res *ingest.SIPNotFound
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewListSipWorkflowsNotFoundResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusNotFound)
			return enc.Encode(body)
		case "forbidden":
			var res ingest.Forbidden
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusForbidden)
			return enc.Encode(body)
		case "unauthorized":
			var res ingest.Unauthorized
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusUnauthorized)
			return enc.Encode(body)
		default:
			return encodeError(ctx, w, v)
		}
	}
}

// EncodeConfirmSipResponse returns an encoder for responses returned by the
// ingest confirm_sip endpoint.
func EncodeConfirmSipResponse(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder) func(context.Context, http.ResponseWriter, any) error {
	return func(ctx context.Context, w http.ResponseWriter, v any) error {
		w.WriteHeader(http.StatusAccepted)
		return nil
	}
}

// DecodeConfirmSipRequest returns a decoder for requests sent to the ingest
// confirm_sip endpoint.
func DecodeConfirmSipRequest(mux goahttp.Muxer, decoder func(*http.Request) goahttp.Decoder) func(*http.Request) (*ingest.ConfirmSipPayload, error) {
	return func(r *http.Request) (*ingest.ConfirmSipPayload, error) {
		var payload *ingest.ConfirmSipPayload
		var (
			body ConfirmSipRequestBody
			err  error
		)
		err = decoder(r).Decode(&body)
		if err != nil {
			if errors.Is(err, io.EOF) {
				return payload, goa.MissingPayloadError()
			}
			var gerr *goa.ServiceError
			if errors.As(err, &gerr) {
				return payload, gerr
			}
			return payload, goa.DecodePayloadError(err.Error())
		}
		err = ValidateConfirmSipRequestBody(&body)
		if err != nil {
			return payload, err
		}

		var (
			uuid  string
			token *string

			params = mux.Vars(r)
		)
		uuid = params["uuid"]
		err = goa.MergeErrors(err, goa.ValidateFormat("uuid", uuid, goa.FormatUUID))
		tokenRaw := r.Header.Get("Authorization")
		if tokenRaw != "" {
			token = &tokenRaw
		}
		if err != nil {
			return payload, err
		}
		payload = NewConfirmSipPayload(&body, uuid, token)
		if payload.Token != nil {
			if strings.Contains(*payload.Token, " ") {
				// Remove authorization scheme prefix (e.g. "Bearer")
				cred := strings.SplitN(*payload.Token, " ", 2)[1]
				payload.Token = &cred
			}
		}

		return payload, nil
	}
}

// EncodeConfirmSipError returns an encoder for errors returned by the
// confirm_sip ingest endpoint.
func EncodeConfirmSipError(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder, formatter func(ctx context.Context, err error) goahttp.Statuser) func(context.Context, http.ResponseWriter, error) error {
	encodeError := goahttp.ErrorEncoder(encoder, formatter)
	return func(ctx context.Context, w http.ResponseWriter, v error) error {
		var en goa.GoaErrorNamer
		if !errors.As(v, &en) {
			return encodeError(ctx, w, v)
		}
		switch en.GoaErrorName() {
		case "not_available":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewConfirmSipNotAvailableResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusConflict)
			return enc.Encode(body)
		case "not_valid":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewConfirmSipNotValidResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusBadRequest)
			return enc.Encode(body)
		case "not_found":
			var res *ingest.SIPNotFound
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewConfirmSipNotFoundResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusNotFound)
			return enc.Encode(body)
		case "forbidden":
			var res ingest.Forbidden
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusForbidden)
			return enc.Encode(body)
		case "unauthorized":
			var res ingest.Unauthorized
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusUnauthorized)
			return enc.Encode(body)
		default:
			return encodeError(ctx, w, v)
		}
	}
}

// EncodeRejectSipResponse returns an encoder for responses returned by the
// ingest reject_sip endpoint.
func EncodeRejectSipResponse(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder) func(context.Context, http.ResponseWriter, any) error {
	return func(ctx context.Context, w http.ResponseWriter, v any) error {
		w.WriteHeader(http.StatusAccepted)
		return nil
	}
}

// DecodeRejectSipRequest returns a decoder for requests sent to the ingest
// reject_sip endpoint.
func DecodeRejectSipRequest(mux goahttp.Muxer, decoder func(*http.Request) goahttp.Decoder) func(*http.Request) (*ingest.RejectSipPayload, error) {
	return func(r *http.Request) (*ingest.RejectSipPayload, error) {
		var payload *ingest.RejectSipPayload
		var (
			uuid  string
			token *string
			err   error

			params = mux.Vars(r)
		)
		uuid = params["uuid"]
		err = goa.MergeErrors(err, goa.ValidateFormat("uuid", uuid, goa.FormatUUID))
		tokenRaw := r.Header.Get("Authorization")
		if tokenRaw != "" {
			token = &tokenRaw
		}
		if err != nil {
			return payload, err
		}
		payload = NewRejectSipPayload(uuid, token)
		if payload.Token != nil {
			if strings.Contains(*payload.Token, " ") {
				// Remove authorization scheme prefix (e.g. "Bearer")
				cred := strings.SplitN(*payload.Token, " ", 2)[1]
				payload.Token = &cred
			}
		}

		return payload, nil
	}
}

// EncodeRejectSipError returns an encoder for errors returned by the
// reject_sip ingest endpoint.
func EncodeRejectSipError(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder, formatter func(ctx context.Context, err error) goahttp.Statuser) func(context.Context, http.ResponseWriter, error) error {
	encodeError := goahttp.ErrorEncoder(encoder, formatter)
	return func(ctx context.Context, w http.ResponseWriter, v error) error {
		var en goa.GoaErrorNamer
		if !errors.As(v, &en) {
			return encodeError(ctx, w, v)
		}
		switch en.GoaErrorName() {
		case "not_available":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewRejectSipNotAvailableResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusConflict)
			return enc.Encode(body)
		case "not_valid":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewRejectSipNotValidResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusBadRequest)
			return enc.Encode(body)
		case "not_found":
			var res *ingest.SIPNotFound
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewRejectSipNotFoundResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusNotFound)
			return enc.Encode(body)
		case "forbidden":
			var res ingest.Forbidden
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusForbidden)
			return enc.Encode(body)
		case "unauthorized":
			var res ingest.Unauthorized
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusUnauthorized)
			return enc.Encode(body)
		default:
			return encodeError(ctx, w, v)
		}
	}
}

// EncodeShowSipDecisionResponse returns an encoder for responses returned by
// the ingest show_sip_decision endpoint.
func EncodeShowSipDecisionResponse(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder) func(context.Context, http.ResponseWriter, any) error {
	return func(ctx context.Context, w http.ResponseWriter, v any) error {
		res := v.(*ingestviews.SIPDecision)
		enc := encoder(ctx, w)
		body := NewShowSipDecisionResponseBody(res.Projected)
		w.WriteHeader(http.StatusOK)
		return enc.Encode(body)
	}
}

// DecodeShowSipDecisionRequest returns a decoder for requests sent to the
// ingest show_sip_decision endpoint.
func DecodeShowSipDecisionRequest(mux goahttp.Muxer, decoder func(*http.Request) goahttp.Decoder) func(*http.Request) (*ingest.ShowSipDecisionPayload, error) {
	return func(r *http.Request) (*ingest.ShowSipDecisionPayload, error) {
		var payload *ingest.ShowSipDecisionPayload
		var (
			uuid  string
			token *string
			err   error

			params = mux.Vars(r)
		)
		uuid = params["uuid"]
		err = goa.MergeErrors(err, goa.ValidateFormat("uuid", uuid, goa.FormatUUID))
		tokenRaw := r.Header.Get("Authorization")
		if tokenRaw != "" {
			token = &tokenRaw
		}
		if err != nil {
			return payload, err
		}
		payload = NewShowSipDecisionPayload(uuid, token)
		if payload.Token != nil {
			if strings.Contains(*payload.Token, " ") {
				// Remove authorization scheme prefix (e.g. "Bearer")
				cred := strings.SplitN(*payload.Token, " ", 2)[1]
				payload.Token = &cred
			}
		}

		return payload, nil
	}
}

// EncodeShowSipDecisionError returns an encoder for errors returned by the
// show_sip_decision ingest endpoint.
func EncodeShowSipDecisionError(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder, formatter func(ctx context.Context, err error) goahttp.Statuser) func(context.Context, http.ResponseWriter, error) error {
	encodeError := goahttp.ErrorEncoder(encoder, formatter)
	return func(ctx context.Context, w http.ResponseWriter, v error) error {
		var en goa.GoaErrorNamer
		if !errors.As(v, &en) {
			return encodeError(ctx, w, v)
		}
		switch en.GoaErrorName() {
		case "internal_error":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewShowSipDecisionInternalErrorResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusInternalServerError)
			return enc.Encode(body)
		case "not_available":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewShowSipDecisionNotAvailableResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusConflict)
			return enc.Encode(body)
		case "not_valid":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewShowSipDecisionNotValidResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusBadRequest)
			return enc.Encode(body)
		case "not_found":
			var res *ingest.SIPNotFound
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewShowSipDecisionNotFoundResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusNotFound)
			return enc.Encode(body)
		case "forbidden":
			var res ingest.Forbidden
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusForbidden)
			return enc.Encode(body)
		case "unauthorized":
			var res ingest.Unauthorized
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusUnauthorized)
			return enc.Encode(body)
		default:
			return encodeError(ctx, w, v)
		}
	}
}

// EncodeSubmitSipDecisionResponse returns an encoder for responses returned by
// the ingest submit_sip_decision endpoint.
func EncodeSubmitSipDecisionResponse(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder) func(context.Context, http.ResponseWriter, any) error {
	return func(ctx context.Context, w http.ResponseWriter, v any) error {
		w.WriteHeader(http.StatusAccepted)
		return nil
	}
}

// DecodeSubmitSipDecisionRequest returns a decoder for requests sent to the
// ingest submit_sip_decision endpoint.
func DecodeSubmitSipDecisionRequest(mux goahttp.Muxer, decoder func(*http.Request) goahttp.Decoder) func(*http.Request) (*ingest.SubmitSipDecisionPayload, error) {
	return func(r *http.Request) (*ingest.SubmitSipDecisionPayload, error) {
		var payload *ingest.SubmitSipDecisionPayload
		var (
			body SubmitSipDecisionRequestBody
			err  error
		)
		err = decoder(r).Decode(&body)
		if err != nil {
			if errors.Is(err, io.EOF) {
				return payload, goa.MissingPayloadError()
			}
			var gerr *goa.ServiceError
			if errors.As(err, &gerr) {
				return payload, gerr
			}
			return payload, goa.DecodePayloadError(err.Error())
		}
		err = ValidateSubmitSipDecisionRequestBody(&body)
		if err != nil {
			return payload, err
		}

		var (
			uuid  string
			token *string

			params = mux.Vars(r)
		)
		uuid = params["uuid"]
		err = goa.MergeErrors(err, goa.ValidateFormat("uuid", uuid, goa.FormatUUID))
		tokenRaw := r.Header.Get("Authorization")
		if tokenRaw != "" {
			token = &tokenRaw
		}
		if err != nil {
			return payload, err
		}
		payload = NewSubmitSipDecisionPayload(&body, uuid, token)
		if payload.Token != nil {
			if strings.Contains(*payload.Token, " ") {
				// Remove authorization scheme prefix (e.g. "Bearer")
				cred := strings.SplitN(*payload.Token, " ", 2)[1]
				payload.Token = &cred
			}
		}

		return payload, nil
	}
}

// EncodeSubmitSipDecisionError returns an encoder for errors returned by the
// submit_sip_decision ingest endpoint.
func EncodeSubmitSipDecisionError(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder, formatter func(ctx context.Context, err error) goahttp.Statuser) func(context.Context, http.ResponseWriter, error) error {
	encodeError := goahttp.ErrorEncoder(encoder, formatter)
	return func(ctx context.Context, w http.ResponseWriter, v error) error {
		var en goa.GoaErrorNamer
		if !errors.As(v, &en) {
			return encodeError(ctx, w, v)
		}
		switch en.GoaErrorName() {
		case "internal_error":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewSubmitSipDecisionInternalErrorResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusInternalServerError)
			return enc.Encode(body)
		case "not_available":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewSubmitSipDecisionNotAvailableResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusConflict)
			return enc.Encode(body)
		case "not_valid":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewSubmitSipDecisionNotValidResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusBadRequest)
			return enc.Encode(body)
		case "not_found":
			var res *ingest.SIPNotFound
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewSubmitSipDecisionNotFoundResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusNotFound)
			return enc.Encode(body)
		case "forbidden":
			var res ingest.Forbidden
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusForbidden)
			return enc.Encode(body)
		case "unauthorized":
			var res ingest.Unauthorized
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusUnauthorized)
			return enc.Encode(body)
		default:
			return encodeError(ctx, w, v)
		}
	}
}

// EncodeAddSipResponse returns an encoder for responses returned by the ingest
// add_sip endpoint.
func EncodeAddSipResponse(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder) func(context.Context, http.ResponseWriter, any) error {
	return func(ctx context.Context, w http.ResponseWriter, v any) error {
		res, _ := v.(*ingest.AddSipResult)
		enc := encoder(ctx, w)
		body := NewAddSipResponseBody(res)
		w.WriteHeader(http.StatusCreated)
		return enc.Encode(body)
	}
}

// DecodeAddSipRequest returns a decoder for requests sent to the ingest
// add_sip endpoint.
func DecodeAddSipRequest(mux goahttp.Muxer, decoder func(*http.Request) goahttp.Decoder) func(*http.Request) (*ingest.AddSipPayload, error) {
	return func(r *http.Request) (*ingest.AddSipPayload, error) {
		var payload *ingest.AddSipPayload
		var (
			sourceID string
			key      string
			token    *string
			err      error
		)
		qp := r.URL.Query()
		sourceID = qp.Get("source_id")
		if sourceID == "" {
			err = goa.MergeErrors(err, goa.MissingFieldError("source_id", "query string"))
		}
		err = goa.MergeErrors(err, goa.ValidateFormat("source_id", sourceID, goa.FormatUUID))
		key = qp.Get("key")
		if key == "" {
			err = goa.MergeErrors(err, goa.MissingFieldError("key", "query string"))
		}
		tokenRaw := r.Header.Get("Authorization")
		if tokenRaw != "" {
			token = &tokenRaw
		}
		if err != nil {
			return payload, err
		}
		payload = NewAddSipPayload(sourceID, key, token)
		if payload.Token != nil {
			if strings.Contains(*payload.Token, " ") {
				// Remove authorization scheme prefix (e.g. "Bearer")
				cred := strings.SplitN(*payload.Token, " ", 2)[1]
				payload.Token = &cred
			}
		}

		return payload, nil
	}
}

// EncodeAddSipError returns an encoder for errors returned by the add_sip
// ingest endpoint.
func EncodeAddSipError(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder, formatter func(ctx context.Context, err error) goahttp.Statuser) func(context.Context, http.ResponseWriter, error) error {
	encodeError := goahttp.ErrorEncoder(encoder, formatter)
	return func(ctx context.Context, w http.ResponseWriter, v error) error {
		var en goa.GoaErrorNamer
		if !errors.As(v, &en) {
			return encodeError(ctx, w, v)
		}
		switch en.GoaErrorName() {
		case "not_valid":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewAddSipNotValidResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusBadRequest)
			return enc.Encode(body)
		case "internal_error":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewAddSipInternalErrorResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusInternalServerError)
			return enc.Encode(body)
		case "forbidden":
			var res ingest.Forbidden
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusForbidden)
			return enc.Encode(body)
		case "unauthorized":
			var res ingest.Unauthorized
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusUnauthorized)
			return enc.Encode(body)
		default:
			return encodeError(ctx, w, v)
		}
	}
}

// EncodeUploadSipResponse returns an encoder for responses returned by the
// ingest upload_sip endpoint.
func EncodeUploadSipResponse(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder) func(context.Context, http.ResponseWriter, any) error {
	return func(ctx context.Context, w http.ResponseWriter, v any) error {
		res, _ := v.(*ingest.UploadSipResult)
		enc := encoder(ctx, w)
		body := NewUploadSipResponseBody(res)
		w.WriteHeader(http.StatusAccepted)
		return enc.Encode(body)
	}
}

// DecodeUploadSipRequest returns a decoder for requests sent to the ingest
// upload_sip endpoint.
func DecodeUploadSipRequest(mux goahttp.Muxer, decoder func(*http.Request) goahttp.Decoder) func(*http.Request) (*ingest.UploadSipPayload, error) {
	return func(r *http.Request) (*ingest.UploadSipPayload, error) {
		var payload *ingest.UploadSipPayload
		var (
			contentType string
			token       *string
			err         error
		)
		contentTypeRaw := r.Header.Get("Content-Type")
		if contentTypeRaw != "" {
			contentType = contentTypeRaw
		} else {
			contentType = "multipart/form-data; boundary=goa"
		}
		err = goa.MergeErrors(err, goa.ValidatePattern("content_type", contentType, "multipart/[^;]+; boundary=.+"))
		tokenRaw := r.Header.Get("Authorization")
		if tokenRaw != "" {
			token = &tokenRaw
		}
		if err != nil {
			return payload, err
		}
		payload = NewUploadSipPayload(contentType, token)
		if payload.Token != nil {
			if strings.Contains(*payload.Token, " ") {
				// Remove authorization scheme prefix (e.g. "Bearer")
				cred := strings.SplitN(*payload.Token, " ", 2)[1]
				payload.Token = &cred
			}
		}

		return payload, nil
	}
}

// EncodeUploadSipError returns an encoder for errors returned by the
// upload_sip ingest endpoint.
func EncodeUploadSipError(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder, formatter func(ctx context.Context, err error) goahttp.Statuser) func(context.Context, http.ResponseWriter, error) error {
	encodeError := goahttp.ErrorEncoder(encoder, formatter)
	return func(ctx context.Context, w http.ResponseWriter, v error) error {
		var en goa.GoaErrorNamer
		if !errors.As(v, &en) {
			return encodeError(ctx, w, v)
		}
		switch en.GoaErrorName() {
		case "invalid_media_type":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewUploadSipInvalidMediaTypeResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusBadRequest)
			return enc.Encode(body)
		case "invalid_multipart_request":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewUploadSipInvalidMultipartRequestResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusBadRequest)
			return enc.Encode(body)
		case "internal_error":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewUploadSipInternalErrorResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusInternalServerError)
			return enc.Encode(body)
		case "forbidden":
			var res ingest.Forbidden
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusForbidden)
			return enc.Encode(body)
		case "unauthorized":
			var res ingest.Unauthorized
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusUnauthorized)
			return enc.Encode(body)
		default:
			return encodeError(ctx, w, v)
		}
	}
}

// EncodeDownloadSipRequestResponse returns an encoder for responses returned
// by the ingest download_sip_request endpoint.
func EncodeDownloadSipRequestResponse(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder) func(context.Context, http.ResponseWriter, any) error {
	return func(ctx context.Context, w http.ResponseWriter, v any) error {
		res, _ := v.(*ingest.DownloadSipRequestResult)
		if res.Ticket != nil {
			ticket := *res.Ticket
			http.SetCookie(w, &http.Cookie{
				Name:     "enduro-sip-download-ticket",
				Value:    ticket,
				MaxAge:   5,
				Secure:   true,
				HttpOnly: true,
			})
		}
		w.WriteHeader(http.StatusOK)
		return nil
	}
}

// DecodeDownloadSipRequestRequest returns a decoder for requests sent to the
// ingest download_sip_request endpoint.
func DecodeDownloadSipRequestRequest(mux goahttp.Muxer, decoder func(*http.Request) goahttp.Decoder) func(*http.Request) (*ingest.DownloadSipRequestPayload, error) {
	return func(r *http.Request) (*ingest.DownloadSipRequestPayload, error) {
		var payload *ingest.DownloadSipRequestPayload
		var (
			uuid  string
			token *string
			err   error

			params = mux.Vars(r)
		)
		uuid = params["uuid"]
		err = goa.MergeErrors(err, goa.ValidateFormat("uuid", uuid, goa.FormatUUID))
		tokenRaw := r.Header.Get("Authorization")
		if tokenRaw != "" {
			token = &tokenRaw
		}
		if err != nil {
			return payload, err
		}
		payload = NewDownloadSipRequestPayload(uuid, token)
		if payload.Token != nil {
			if strings.Contains(*payload.Token, " ") {
				// Remove authorization scheme prefix (e.g. "Bearer")
				cred := strings.SplitN(*payload.Token, " ", 2)[1]
				payload.Token = &cred
			}
		}

		return payload, nil
	}
}

// EncodeDownloadSipRequestError returns an encoder for errors returned by the
// download_sip_request ingest endpoint.
func EncodeDownloadSipRequestError(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder, formatter func(ctx context.Context, err error) goahttp.Statuser) func(context.Context, http.ResponseWriter, error) error {
	encodeError := goahttp.ErrorEncoder(encoder, formatter)
	return func(ctx context.Context, w http.ResponseWriter, v error) error {
		var en goa.GoaErrorNamer
		if !errors.As(v, &en) {
			return encodeError(ctx, w, v)
		}
		switch en.GoaErrorName() {
		case "not_valid":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewDownloadSipRequestNotValidResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusBadRequest)
			return enc.Encode(body)
		case "internal_error":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewDownloadSipRequestInternalErrorResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusInternalServerError)
			return enc.Encode(body)
		case "not_found":
			var res *ingest.SIPNotFound
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewDownloadSipRequestNotFoundResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusNotFound)
			return enc.Encode(body)
		case "forbidden":
			var res ingest.Forbidden
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusForbidden)
			return enc.Encode(body)
		case "unauthorized":
			var res ingest.Unauthorized
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusUnauthorized)
			return enc.Encode(body)
		default:
			return encodeError(ctx, w, v)
		}
	}
}

// EncodeDownloadSipResponse returns an encoder for responses returned by the
// ingest download_sip endpoint.
func EncodeDownloadSipResponse(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder) func(context.Context, http.ResponseWriter, any) error {
	return func(ctx context.Context, w http.ResponseWriter, v any) error {
		res, _ := v.(*ingest.DownloadSipResult)
		w.Header().Set("Content-Type", res.ContentType)
		{
			val := res.ContentLength
			contentLengths := strconv.FormatInt(val, 10)
			w.Header().Set("Content-Length", contentLengths)
		}
		w.Header().Set("Content-Disposition", res.ContentDisposition)
		w.WriteHeader(http.StatusOK)
		return nil
	}
}

// DecodeDownloadSipRequest returns a decoder for requests sent to the ingest
// download_sip endpoint.
func DecodeDownloadSipRequest(mux goahttp.Muxer, decoder func(*http.Request) goahttp.Decoder) func(*http.Request) (*ingest.DownloadSipPayload, error) {
	return func(r *http.Request) (*ingest.DownloadSipPayload, error) {
		var payload *ingest.DownloadSipPayload
		var (
			uuid   string
			ticket *string
			err    error
			c      *http.Cookie

			params = mux.Vars(r)
		)
		uuid = params["uuid"]
		err = goa.MergeErrors(err, goa.ValidateFormat("uuid", uuid, goa.FormatUUID))
		c, _ = r.Cookie("enduro-sip-download-ticket")
		var ticketRaw string
		if c != nil {
			ticketRaw = c.Value
		}
		if ticketRaw != "" {
			ticket = &ticketRaw
		}
		if err != nil {
			return payload, err
		}
		payload = NewDownloadSipPayload(uuid, ticket)

		return payload, nil
	}
}

// EncodeDownloadSipError returns an encoder for errors returned by the
// download_sip ingest endpoint.
func EncodeDownloadSipError(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder, formatter func(ctx context.Context, err error) goahttp.Statuser) func(context.Context, http.ResponseWriter, error) error {
	encodeError := goahttp.ErrorEncoder(encoder, formatter)
	return func(ctx context.Context, w http.ResponseWriter, v error) error {
		var en goa.GoaErrorNamer
		if !errors.As(v, &en) {
			return encodeError(ctx, w, v)
		}
		switch en.GoaErrorName() {
		case "not_valid":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewDownloadSipNotValidResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusBadRequest)
			return enc.Encode(body)
		case "internal_error":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewDownloadSipInternalErrorResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusInternalServerError)
			return enc.Encode(body)
		case "not_found":
			var res *ingest.SIPNotFound
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewDownloadSipNotFoundResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusNotFound)
			return enc.Encode(body)
		case "forbidden":
			var res ingest.Forbidden
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusForbidden)
			return enc.Encode(body)
		case "unauthorized":
			var res ingest.Unauthorized
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusUnauthorized)
			return enc.Encode(body)
		default:
			return encodeError(ctx, w, v)
		}
	}
}

// EncodeListUsersResponse returns an encoder for responses returned by the
// ingest list_users endpoint.
func EncodeListUsersResponse(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder) func(context.Context, http.ResponseWriter, any) error {
	return func(ctx context.Context, w http.ResponseWriter, v any) error {
		res := v.(*ingestviews.Users)
		enc := encoder(ctx, w)
		body := NewListUsersResponseBody(res.Projected)
		w.WriteHeader(http.StatusOK)
		return enc.Encode(body)
	}
}

// DecodeListUsersRequest returns a decoder for requests sent to the ingest
// list_users endpoint.
func DecodeListUsersRequest(mux goahttp.Muxer, decoder func(*http.Request) goahttp.Decoder) func(*http.Request) (*ingest.ListUsersPayload, error) {
	return func(r *http.Request) (*ingest.ListUsersPayload, error) {
		var payload *ingest.ListUsersPayload
		var (
			email  *string
			name   *string
			limit  *int
			offset *int
			token  *string
			err    error
		)
		qp := r.URL.Query()
		emailRaw := qp.Get("email")
		if emailRaw != "" {
			email = &emailRaw
		}
		nameRaw := qp.Get("name")
		if nameRaw != "" {
			name = &nameRaw
		}
		{
			limitRaw := qp.Get("limit")
			if limitRaw != "" {
				v, err2 := strconv.ParseInt(limitRaw, 10, strconv.IntSize)
				if err2 != nil {
					err = goa.MergeErrors(err, goa.InvalidFieldTypeError("limit", limitRaw, "integer"))
				}
				pv := int(v)
				limit = &pv
			}
		}
		{
			offsetRaw := qp.Get("offset")
			if offsetRaw != "" {
				v, err2 := strconv.ParseInt(offsetRaw, 10, strconv.IntSize)
				if err2 != nil {
					err = goa.MergeErrors(err, goa.InvalidFieldTypeError("offset", offsetRaw, "integer"))
				}
				pv := int(v)
				offset = &pv
			}
		}
		tokenRaw := r.Header.Get("Authorization")
		if tokenRaw != "" {
			token = &tokenRaw
		}
		if err != nil {
			return payload, err
		}
		payload = NewListUsersPayload(email, name, limit, offset, token)
		if payload.Token != nil {
			if strings.Contains(*payload.Token, " ") {
				// Remove authorization scheme prefix (e.g. "Bearer")
				cred := strings.SplitN(*payload.Token, " ", 2)[1]
				payload.Token = &cred
			}
		}

		return payload, nil
	}
}

// EncodeListUsersError returns an encoder for errors returned by the
// list_users ingest endpoint.
func EncodeListUsersError(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder, formatter func(ctx context.Context, err error) goahttp.Statuser) func(context.Context, http.ResponseWriter, error) error {
	encodeError := goahttp.ErrorEncoder(encoder, formatter)
	return func(ctx context.Context, w http.ResponseWriter, v error) error {
		var en goa.GoaErrorNamer
		if !errors.As(v, &en) {
			return encodeError(ctx, w, v)
		}
		switch en.GoaErrorName() {
		case "not_valid":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewListUsersNotValidResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusBadRequest)
			return enc.Encode(body)
		case "forbidden":
			var res ingest.Forbidden
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusForbidden)
			return enc.Encode(body)
		case "unauthorized":
			var res ingest.Unauthorized
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusUnauthorized)
			return enc.Encode(body)
		default:
			return encodeError(ctx, w, v)
		}
	}
}

// EncodeListSipSourceObjectsResponse returns an encoder for responses returned
// by the ingest list_sip_source_objects endpoint.
func EncodeListSipSourceObjectsResponse(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder) func(context.Context, http.ResponseWriter, any) error {
	return func(ctx context.Context, w http.ResponseWriter, v any) error {
		res := v.(*ingestviews.SIPSourceObjects)
		enc := encoder(ctx, w)
		body := NewListSipSourceObjectsResponseBody(res.Projected)
		w.WriteHeader(http.StatusOK)
		return enc.Encode(body)
	}
}

// DecodeListSipSourceObjectsRequest returns a decoder for requests sent to the
// ingest list_sip_source_objects endpoint.
func DecodeListSipSourceObjectsRequest(mux goahttp.Muxer, decoder func(*http.Request) goahttp.Decoder) func(*http.Request) (*ingest.ListSipSourceObjectsPayload, error) {
	return func(r *http.Request) (*ingest.ListSipSourceObjectsPayload, error) {
		var payload *ingest.ListSipSourceObjectsPayload
		var (
			uuid   string
			limit  *int
			cursor *string
			token  *string
			err    error

			params = mux.Vars(r)
		)
		uuid = params["uuid"]
		err = goa.MergeErrors(err, goa.ValidateFormat("uuid", uuid, goa.FormatUUID))
		qp := r.URL.Query()
		{
			limitRaw := qp.Get("limit")
			if limitRaw != "" {
				v, err2 := strconv.ParseInt(limitRaw, 10, strconv.IntSize)
				if err2 != nil {
					err = goa.MergeErrors(err, goa.InvalidFieldTypeError("limit", limitRaw, "integer"))
				}
				pv := int(v)
				limit = &pv
			}
		}
		cursorRaw := qp.Get("cursor")
		if cursorRaw != "" {
			cursor = &cursorRaw
		}
		tokenRaw := r.Header.Get("Authorization")
		if tokenRaw != "" {
			token = &tokenRaw
		}
		if err != nil {
			return payload, err
		}
		payload = NewListSipSourceObjectsPayload(uuid, limit, cursor, token)
		if payload.Token != nil {
			if strings.Contains(*payload.Token, " ") {
				// Remove authorization scheme prefix (e.g. "Bearer")
				cred := strings.SplitN(*payload.Token, " ", 2)[1]
				payload.Token = &cred
			}
		}

		return payload, nil
	}
}

// EncodeListSipSourceObjectsError returns an encoder for errors returned by
// the list_sip_source_objects ingest endpoint.
func EncodeListSipSourceObjectsError(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder, formatter func(ctx context.Context, err error) goahttp.Statuser) func(context.Context, http.ResponseWriter, error) error {
	encodeError := goahttp.ErrorEncoder(encoder, formatter)
	return func(ctx context.Context, w http.ResponseWriter, v error) error {
		var en goa.GoaErrorNamer
		if !errors.As(v, &en) {
			return encodeError(ctx, w, v)
		}
		switch en.GoaErrorName() {
		case "not_found":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewListSipSourceObjectsNotFoundResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusNotFound)
			return enc.Encode(body)
		case "not_valid":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewListSipSourceObjectsNotValidResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusBadRequest)
			return enc.Encode(body)
		case "internal_error":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewListSipSourceObjectsInternalErrorResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusInternalServerError)
			return enc.Encode(body)
		case "forbidden":
			var res ingest.Forbidden
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusForbidden)
			return enc.Encode(body)
		case "unauthorized":
			var res ingest.Unauthorized
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusUnauthorized)
			return enc.Encode(body)
		default:
			return encodeError(ctx, w, v)
		}
	}
}

// EncodeAddBatchResponse returns an encoder for responses returned by the
// ingest add_batch endpoint.
func EncodeAddBatchResponse(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder) func(context.Context, http.ResponseWriter, any) error {
	return func(ctx context.Context, w http.ResponseWriter, v any) error {
		res, _ := v.(*ingest.AddBatchResult)
		enc := encoder(ctx, w)
		body := NewAddBatchResponseBody(res)
		w.WriteHeader(http.StatusCreated)
		return enc.Encode(body)
	}
}

// DecodeAddBatchRequest returns a decoder for requests sent to the ingest
// add_batch endpoint.
func DecodeAddBatchRequest(mux goahttp.Muxer, decoder func(*http.Request) goahttp.Decoder) func(*http.Request) (*ingest.AddBatchPayload, error) {
	return func(r *http.Request) (*ingest.AddBatchPayload, error) {
		var payload *ingest.AddBatchPayload
		var (
			body AddBatchRequestBody
			err  error
		)
		err = decoder(r).Decode(&body)
		if err != nil {
			if errors.Is(err, io.EOF) {
				return payload, goa.MissingPayloadError()
			}
			var gerr *goa.ServiceError
			if errors.As(err, &gerr) {
				return payload, gerr
			}
			return payload, goa.DecodePayloadError(err.Error())
		}
		err = ValidateAddBatchRequestBody(&body)
		if err != nil {
			return payload, err
		}

		var (
			token *string
		)
		tokenRaw := r.Header.Get("Authorization")
		if tokenRaw != "" {
			token = &tokenRaw
		}
		payload = NewAddBatchPayload(&body, token)
		if payload.Token != nil {
			if strings.Contains(*payload.Token, " ") {
				// Remove authorization scheme prefix (e.g. "Bearer")
				cred := strings.SplitN(*payload.Token, " ", 2)[1]
				payload.Token = &cred
			}
		}

		return payload, nil
	}
}

// EncodeAddBatchError returns an encoder for errors returned by the add_batch
// ingest endpoint.
func EncodeAddBatchError(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder, formatter func(ctx context.Context, err error) goahttp.Statuser) func(context.Context, http.ResponseWriter, error) error {
	encodeError := goahttp.ErrorEncoder(encoder, formatter)
	return func(ctx context.Context, w http.ResponseWriter, v error) error {
		var en goa.GoaErrorNamer
		if !errors.As(v, &en) {
			return encodeError(ctx, w, v)
		}
		switch en.GoaErrorName() {
		case "not_valid":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewAddBatchNotValidResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusBadRequest)
			return enc.Encode(body)
		case "internal_error":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewAddBatchInternalErrorResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusInternalServerError)
			return enc.Encode(body)
		case "forbidden":
			var res ingest.Forbidden
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusForbidden)
			return enc.Encode(body)
		case "unauthorized":
			var res ingest.Unauthorized
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusUnauthorized)
			return enc.Encode(body)
		default:
			return encodeError(ctx, w, v)
		}
	}
}

// EncodeListBatchesResponse returns an encoder for responses returned by the
// ingest list_batches endpoint.
func EncodeListBatchesResponse(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder) func(context.Context, http.ResponseWriter, any) error {
	return func(ctx context.Context, w http.ResponseWriter, v any) error {
		res := v.(*ingestviews.Batches)
		enc := encoder(ctx, w)
		body := NewListBatchesResponseBody(res.Projected)
		w.WriteHeader(http.StatusOK)
		return enc.Encode(body)
	}
}

// DecodeListBatchesRequest returns a decoder for requests sent to the ingest
// list_batches endpoint.
func DecodeListBatchesRequest(mux goahttp.Muxer, decoder func(*http.Request) goahttp.Decoder) func(*http.Request) (*ingest.ListBatchesPayload, error) {
	return func(r *http.Request) (*ingest.ListBatchesPayload, error) {
		var payload *ingest.ListBatchesPayload
		var (
			identifier          *string
			earliestCreatedTime *string
			latestCreatedTime   *string
			status              *string
			uploaderUUID        *string
			limit               *int
			offset              *int
			token               *string
			err                 error
		)
		qp := r.URL.Query()
		identifierRaw := qp.Get("identifier")
		if identifierRaw != "" {
			identifier = &identifierRaw
		}
		earliestCreatedTimeRaw := qp.Get("earliest_created_time")
		if earliestCreatedTimeRaw != "" {
			earliestCreatedTime = &earliestCreatedTimeRaw
		}
		if earliestCreatedTime != nil {
			err = goa.MergeErrors(err, goa.ValidateFormat("earliest_created_time", *earliestCreatedTime, goa.FormatDateTime))
		}
		latestCreatedTimeRaw := qp.Get("latest_created_time")
		if latestCreatedTimeRaw != "" {
			latestCreatedTime = &latestCreatedTimeRaw
		}
		if latestCreatedTime != nil {
			err = goa.MergeErrors(err, goa.ValidateFormat("latest_created_time", *latestCreatedTime, goa.FormatDateTime))
		}
		statusRaw := qp.Get("status")
		if statusRaw != "" {
			status = &statusRaw
		}
		if status != nil {
			if !(*status == "queued" || *status == "processing" || *status == "pending" || *status == "ingested" || *status == "canceled" || *status == "failed") {
				err = goa.MergeErrors(err, goa.InvalidEnumValueError("status", *status, []any{"queued", "processing", "pending", "ingested", "canceled", "failed"}))
			}
		}
		uploaderUUIDRaw := qp.Get("uploader_uuid")
		if uploaderUUIDRaw != "" {
			uploaderUUID = &uploaderUUIDRaw
		}
		if uploaderUUID != nil {
			err = goa.MergeErrors(err, goa.ValidateFormat("uploader_uuid", *uploaderUUID, goa.FormatUUID))
		}
		{
			limitRaw := qp.Get("limit")
			if limitRaw != "" {
				v, err2 := strconv.ParseInt(limitRaw, 10, strconv.IntSize)
				if err2 != nil {
					err = goa.MergeErrors(err, goa.InvalidFieldTypeError("limit", limitRaw, "integer"))
				}
				pv := int(v)
				limit = &pv
			}
		}
		{
			offsetRaw := qp.Get("offset")
			if offsetRaw != "" {
				v, err2 := strconv.ParseInt(offsetRaw, 10, strconv.IntSize)
				if err2 != nil {
					err = goa.MergeErrors(err, goa.InvalidFieldTypeError("offset", offsetRaw, "integer"))
				}
				pv := int(v)
				offset = &pv
			}
		}
		tokenRaw := r.Header.Get("Authorization")
		if tokenRaw != "" {
			token = &tokenRaw
		}
		if err != nil {
			return payload, err
		}
		payload = NewListBatchesPayload(identifier, earliestCreatedTime, latestCreatedTime, status, uploaderUUID, limit, offset, token)
		if payload.Token != nil {
			if strings.Contains(*payload.Token, " ") {
				// Remove authorization scheme prefix (e.g. "Bearer")
				cred := strings.SplitN(*payload.Token, " ", 2)[1]
				payload.Token = &cred
			}
		}

		return payload, nil
	}
}

// EncodeListBatchesError returns an encoder for errors returned by the
// list_batches ingest endpoint.
func EncodeListBatchesError(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder, formatter func(ctx context.Context, err error) goahttp.Statuser) func(context.Context, http.ResponseWriter, error) error {
	encodeError := goahttp.ErrorEncoder(encoder, formatter)
	return func(ctx context.Context, w http.ResponseWriter, v error) error {
		var en goa.GoaErrorNamer
		if !errors.As(v, &en) {
			return encodeError(ctx, w, v)
		}
		switch en.GoaErrorName() {
		case "not_valid":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewListBatchesNotValidResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusBadRequest)
			return enc.Encode(body)
		case "internal_error":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewListBatchesInternalErrorResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusInternalServerError)
			return enc.Encode(body)
		case "forbidden":
			var res ingest.Forbidden
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusForbidden)
			return enc.Encode(body)
		case "unauthorized":
			var res ingest.Unauthorized
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusUnauthorized)
			return enc.Encode(body)
		default:
			return encodeError(ctx, w, v)
		}
	}
}

// EncodeShowBatchResponse returns an encoder for responses returned by the
// ingest show_batch endpoint.
func EncodeShowBatchResponse(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder) func(context.Context, http.ResponseWriter, any) error {
	return func(ctx context.Context, w http.ResponseWriter, v any) error {
		res := v.(*ingestviews.Batch)
		enc := encoder(ctx, w)
		body := NewShowBatchResponseBody(res.Projected)
		w.WriteHeader(http.StatusOK)
		return enc.Encode(body)
	}
}

// DecodeShowBatchRequest returns a decoder for requests sent to the ingest
// show_batch endpoint.
func DecodeShowBatchRequest(mux goahttp.Muxer, decoder func(*http.Request) goahttp.Decoder) func(*http.Request) (*ingest.ShowBatchPayload, error) {
	return func(r *http.Request) (*ingest.ShowBatchPayload, error) {
		var payload *ingest.ShowBatchPayload
		var (
			uuid  string
			token *string
			err   error

			params = mux.Vars(r)
		)
		uuid = params["uuid"]
		err = goa.MergeErrors(err, goa.ValidateFormat("uuid", uuid, goa.FormatUUID))
		tokenRaw := r.Header.Get("Authorization")
		if tokenRaw != "" {
			token = &tokenRaw
		}
		if err != nil {
			return payload, err
		}
		payload = NewShowBatchPayload(uuid, token)
		if payload.Token != nil {
			if strings.Contains(*payload.Token, " ") {
				// Remove authorization scheme prefix (e.g. "Bearer")
				cred := strings.SplitN(*payload.Token, " ", 2)[1]
				payload.Token = &cred
			}
		}

		return payload, nil
	}
}

// EncodeShowBatchError returns an encoder for errors returned by the
// show_batch ingest endpoint.
func EncodeShowBatchError(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder, formatter func(ctx context.Context, err error) goahttp.Statuser) func(context.Context, http.ResponseWriter, error) error {
	encodeError := goahttp.ErrorEncoder(encoder, formatter)
	return func(ctx context.Context, w http.ResponseWriter, v error) error {
		var en goa.GoaErrorNamer
		if !errors.As(v, &en) {
			return encodeError(ctx, w, v)
		}
		switch en.GoaErrorName() {
		case "not_valid":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewShowBatchNotValidResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusBadRequest)
			return enc.Encode(body)
		case "internal_error":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewShowBatchInternalErrorResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusInternalServerError)
			return enc.Encode(body)
		case "not_found":
			var res *ingest.BatchNotFound
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewShowBatchNotFoundResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusNotFound)
			return enc.Encode(body)
		case "forbidden":
			var res ingest.Forbidden
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusForbidden)
			return enc.Encode(body)
		case "unauthorized":
			var res ingest.Unauthorized
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusUnauthorized)
			return enc.Encode(body)
		default:
			return encodeError(ctx, w, v)
		}
	}
}

// EncodeReviewBatchResponse returns an encoder for responses returned by the
// ingest review_batch endpoint.
func EncodeReviewBatchResponse(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder) func(context.Context, http.ResponseWriter, any) error {
	return func(ctx context.Context, w http.ResponseWriter, v any) error {
		w.WriteHeader(http.StatusAccepted)
		return nil
	}
}

// DecodeReviewBatchRequest returns a decoder for requests sent to the ingest
// review_batch endpoint.
func DecodeReviewBatchRequest(mux goahttp.Muxer, decoder func(*http.Request) goahttp.Decoder) func(*http.Request) (*ingest.ReviewBatchPayload, error) {
	return func(r *http.Request) (*ingest.ReviewBatchPayload, error) {
		var payload *ingest.ReviewBatchPayload
		var (
			body ReviewBatchRequestBody
			err  error
		)
		err = decoder(r).Decode(&body)
		if err != nil {
			if errors.Is(err, io.EOF) {
				return payload, goa.MissingPayloadError()
			}
			var gerr *goa.ServiceError
			if errors.As(err, &gerr) {
				return payload, gerr
			}
			return payload, goa.DecodePayloadError(err.Error())
		}
		err = ValidateReviewBatchRequestBody(&body)
		if err != nil {
			return payload, err
		}

		var (
			uuid  string
			token *string

			params = mux.Vars(r)
		)
		uuid = params["uuid"]
		err = goa.MergeErrors(err, goa.ValidateFormat("uuid", uuid, goa.FormatUUID))
		tokenRaw := r.Header.Get("Authorization")
		if tokenRaw != "" {
			token = &tokenRaw
		}
		if err != nil {
			return payload, err
		}
		payload = NewReviewBatchPayload(&body, uuid, token)
		if payload.Token != nil {
			if strings.Contains(*payload.Token, " ") {
				// Remove authorization scheme prefix (e.g. "Bearer")
				cred := strings.SplitN(*payload.Token, " ", 2)[1]
				payload.Token = &cred
			}
		}

		return payload, nil
	}
}

// EncodeReviewBatchError returns an encoder for errors returned by the
// review_batch ingest endpoint.
func EncodeReviewBatchError(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder, formatter func(ctx context.Context, err error) goahttp.Statuser) func(context.Context, http.ResponseWriter, error) error {
	encodeError := goahttp.ErrorEncoder(encoder, formatter)
	return func(ctx context.Context, w http.ResponseWriter, v error) error {
		var en goa.GoaErrorNamer
		if !errors.As(v, &en) {
			return encodeError(ctx, w, v)
		}
		switch en.GoaErrorName() {
		case "not_valid":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewReviewBatchNotValidResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusBadRequest)
			return enc.Encode(body)
		case "internal_error":
			var res *goa.ServiceError
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewReviewBatchInternalErrorResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusInternalServerError)
			return enc.Encode(body)
		case "not_found":
			var res *ingest.BatchNotFound
			errors.As(v, &res)
			enc := encoder(ctx, w)
			var body any
			if formatter != nil {
				body = formatter(ctx, res)
			} else {
				body = NewReviewBatchNotFoundResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusNotFound)
			return enc.Encode(body)
		case "forbidden":
			var res ingest.Forbidden
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusForbidden)
			return enc.Encode(body)
		case "unauthorized":
			var res ingest.Unauthorized
			errors.As(v, &res)
			enc := encoder(ctx, w)
			body := res
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusUnauthorized)
			return enc.Encode(body)
		default:
			return encodeError(ctx, w, v)
		}
	}
}

// marshalIngestIngestPingEventToIngestPingEventResponseBody builds a value of
// type *IngestPingEventResponseBody from a value of type
// *ingest.IngestPingEvent.
func marshalIngestIngestPingEventToIngestPingEventResponseBody(v *ingest.IngestPingEvent) *IngestPingEventResponseBody {
	if v == nil {
		return nil
	}
	res := &IngestPingEventResponseBody{
		Message: v.Message,
	}

	return res
}

// marshalIngestSIPCreatedEventToSIPCreatedEventResponseBody builds a value of
// type *SIPCreatedEventResponseBody from a value of type
// *ingest.SIPCreatedEvent.
func marshalIngestSIPCreatedEventToSIPCreatedEventResponseBody(v *ingest.SIPCreatedEvent) *SIPCreatedEventResponseBody {
	if v == nil {
		return nil
	}
	res := &SIPCreatedEventResponseBody{
		UUID: v.UUID,
	}
	if v.Item != nil {
		res.Item = marshalIngestSIPToSIPResponseBody(v.Item)
	}

	return res
}

// marshalIngestSIPToSIPResponseBody builds a value of type *SIPResponseBody
// from a value of type *ingest.SIP.
func marshalIngestSIPToSIPResponseBody(v *ingest.SIP) *SIPResponseBody {
	res := &SIPResponseBody{
		UUID:            v.UUID,
		Name:            v.Name,
		Status:          v.Status,
		AipUUID:         v.AipUUID,
		CreatedAt:       v.CreatedAt,
		StartedAt:       v.StartedAt,
		CompletedAt:     v.CompletedAt,
		FailedAs:        v.FailedAs,
		FailedKey:       v.FailedKey,
		UploaderUUID:    v.UploaderUUID,
		UploaderEmail:   v.UploaderEmail,
		UploaderName:    v.UploaderName,
		BatchUUID:       v.BatchUUID,
		BatchIdentifier: v.BatchIdentifier,
		BatchStatus:     v.BatchStatus,
		FileCount:       v.FileCount,
	}

	return res
}

// marshalIngestSIPUpdatedEventToSIPUpdatedEventResponseBody builds a value of
// type *SIPUpdatedEventResponseBody from a value of type
// *ingest.SIPUpdatedEvent.
func marshalIngestSIPUpdatedEventToSIPUpdatedEventResponseBody(v *ingest.SIPUpdatedEvent) *SIPUpdatedEventResponseBody {
	if v == nil {
		return nil
	}
	res := &SIPUpdatedEventResponseBody{
		UUID: v.UUID,
	}
	if v.Item != nil {
		res.Item = marshalIngestSIPToSIPResponseBody(v.Item)
	}

	return res
}

// marshalIngestSIPStatusUpdatedEventToSIPStatusUpdatedEventResponseBody builds
// a value of type *SIPStatusUpdatedEventResponseBody from a value of type
// *ingest.SIPStatusUpdatedEvent.
func marshalIngestSIPStatusUpdatedEventToSIPStatusUpdatedEventResponseBody(v *ingest.SIPStatusUpdatedEvent) *SIPStatusUpdatedEventResponseBody {
	if v == nil {
		return nil
	}
	res := &SIPStatusUpdatedEventResponseBody{
		UUID:   v.UUID,
		Status: v.Status,
	}

	return res
}

// marshalIngestSIPWorkflowCreatedEventToSIPWorkflowCreatedEventResponseBody
// builds a value of type *SIPWorkflowCreatedEventResponseBody from a value of
// type *ingest.SIPWorkflowCreatedEvent.
func marshalIngestSIPWorkflowCreatedEventToSIPWorkflowCreatedEventResponseBody(v *ingest.SIPWorkflowCreatedEvent) *SIPWorkflowCreatedEventResponseBody {
	if v == nil {
		return nil
	}
	res := &SIPWorkflowCreatedEventResponseBody{
		UUID: v.UUID,
	}
	if v.Item != nil {
		res.Item = marshalIngestSIPWorkflowToSIPWorkflowResponseBody(v.Item)
	}

	return res
}

// marshalIngestSIPWorkflowToSIPWorkflowResponseBody builds a value of type
// *SIPWorkflowResponseBody from a value of type *ingest.SIPWorkflow.
func marshalIngestSIPWorkflowToSIPWorkflowResponseBody(v *ingest.SIPWorkflow) *SIPWorkflowResponseBody {
	res := &SIPWorkflowResponseBody{
		UUID:        v.UUID,
		TemporalID:  v.TemporalID,
		Type:        v.Type,
		Status:      v.Status,
		StartedAt:   v.StartedAt,
		CompletedAt: v.CompletedAt,
		SipUUID:     v.SipUUID,
	}
	if v.Tasks != nil {
		res.Tasks = make([]*SIPTaskResponseBody, len(v.Tasks))
		for i, val := range v.Tasks {
			if val == nil {
				res.Tasks[i] = nil
				continue
			}
			res.Tasks[i] = marshalIngestSIPTaskToSIPTaskResponseBody(val)
		}
	}

	return res
}

// marshalIngestSIPTaskToSIPTaskResponseBody builds a value of type
// *SIPTaskResponseBody from a value of type *ingest.SIPTask.
func marshalIngestSIPTaskToSIPTaskResponseBody(v *ingest.SIPTask) *SIPTaskResponseBody {
	if v == nil {
		return nil
	}
	res := &SIPTaskResponseBody{
		UUID:         v.UUID,
		Name:         v.Name,
		Status:       v.Status,
		StartedAt:    v.StartedAt,
		CompletedAt:  v.CompletedAt,
		Note:         v.Note,
		WorkflowUUID: v.WorkflowUUID,
	}

	return res
}

// marshalIngestSIPWorkflowUpdatedEventToSIPWorkflowUpdatedEventResponseBody
// builds a value of type *SIPWorkflowUpdatedEventResponseBody from a value of
// type *ingest.SIPWorkflowUpdatedEvent.
func marshalIngestSIPWorkflowUpdatedEventToSIPWorkflowUpdatedEventResponseBody(v *ingest.SIPWorkflowUpdatedEvent) *SIPWorkflowUpdatedEventResponseBody {
	if v == nil {
		return nil
	}
	res := &SIPWorkflowUpdatedEventResponseBody{
		UUID: v.UUID,
	}
	if v.Item != nil {
		res.Item = marshalIngestSIPWorkflowToSIPWorkflowResponseBody(v.Item)
	}

	return res
}

// marshalIngestSIPTaskCreatedEventToSIPTaskCreatedEventResponseBody builds a
// value of type *SIPTaskCreatedEventResponseBody from a value of type
// *ingest.SIPTaskCreatedEvent.
func marshalIngestSIPTaskCreatedEventToSIPTaskCreatedEventResponseBody(v *ingest.SIPTaskCreatedEvent) *SIPTaskCreatedEventResponseBody {
	if v == nil {
		return nil
	}
	res := &SIPTaskCreatedEventResponseBody{
		UUID: v.UUID,
	}
	if v.Item != nil {
		res.Item = marshalIngestSIPTaskToSIPTaskResponseBody(v.Item)
	}

	return res
}

// marshalIngestSIPTaskUpdatedEventToSIPTaskUpdatedEventResponseBody builds a
// value of type *SIPTaskUpdatedEventResponseBody from a value of type
// *ingest.SIPTaskUpdatedEvent.
func marshalIngestSIPTaskUpdatedEventToSIPTaskUpdatedEventResponseBody(v *ingest.SIPTaskUpdatedEvent) *SIPTaskUpdatedEventResponseBody {
	if v == nil {
		return nil
	}
	res := &SIPTaskUpdatedEventResponseBody{
		UUID: v.UUID,
	}
	if v.Item != nil {
		res.Item = marshalIngestSIPTaskToSIPTaskResponseBody(v.Item)
	}

	return res
}

// marshalIngestBatchCreatedEventToBatchCreatedEventResponseBody builds a value
// of type *BatchCreatedEventResponseBody from a value of type
// *ingest.BatchCreatedEvent.
func marshalIngestBatchCreatedEventToBatchCreatedEventResponseBody(v *ingest.BatchCreatedEvent) *BatchCreatedEventResponseBody {
	if v == nil {
		return nil
	}
	res := &BatchCreatedEventResponseBody{
		UUID: v.UUID,
	}
	if v.Item != nil {
		res.Item = marshalIngestBatchToBatchResponseBody(v.Item)
	}

	return res
}

// marshalIngestBatchToBatchResponseBody builds a value of type
// *BatchResponseBody from a value of type *ingest.Batch.
func marshalIngestBatchToBatchResponseBody(v *ingest.Batch) *BatchResponseBody {
	res := &BatchResponseBody{
		UUID:          v.UUID,
		Identifier:    v.Identifier,
		SipsCount:     v.SipsCount,
		Status:        v.Status,
		CreatedAt:     v.CreatedAt,
		StartedAt:     v.StartedAt,
		CompletedAt:   v.CompletedAt,
		UploaderUUID:  v.UploaderUUID,
		UploaderEmail: v.UploaderEmail,
		UploaderName:  v.UploaderName,
	}

	return res
}

// marshalIngestBatchUpdatedEventToBatchUpdatedEventResponseBody builds a value
// of type *BatchUpdatedEventResponseBody from a value of type
// *ingest.BatchUpdatedEvent.
func marshalIngestBatchUpdatedEventToBatchUpdatedEventResponseBody(v *ingest.BatchUpdatedEvent) *BatchUpdatedEventResponseBody {
	if v == nil {
		return nil
	}
	res := &BatchUpdatedEventResponseBody{
		UUID: v.UUID,
	}
	if v.Item != nil {
		res.Item = marshalIngestBatchToBatchResponseBody(v.Item)
	}

	return res
}

// marshalIngestviewsSIPViewToSIPResponseBody builds a value of type
// *SIPResponseBody from a value of type *ingestviews.SIPView.
func marshalIngestviewsSIPViewToSIPResponseBody(v *ingestviews.SIPView) *SIPResponseBody {
	res := &SIPResponseBody{
		UUID:            *v.UUID,
		Name:            v.Name,
		Status:          *v.Status,
		AipUUID:         v.AipUUID,
		CreatedAt:       *v.CreatedAt,
		StartedAt:       v.StartedAt,
		CompletedAt:     v.CompletedAt,
		FailedAs:        v.FailedAs,
		FailedKey:       v.FailedKey,
		UploaderUUID:    v.UploaderUUID,
		UploaderEmail:   v.UploaderEmail,
		UploaderName:    v.UploaderName,
		BatchUUID:       v.BatchUUID,
		BatchIdentifier: v.BatchIdentifier,
		BatchStatus:     v.BatchStatus,
		FileCount:       v.FileCount,
	}

	return res
}

// marshalIngestviewsEnduroPageViewToEnduroPageResponseBody builds a value of
// type *EnduroPageResponseBody from a value of type
// *ingestviews.EnduroPageView.
func marshalIngestviewsEnduroPageViewToEnduroPageResponseBody(v *ingestviews.EnduroPageView) *EnduroPageResponseBody {
	res := &EnduroPageResponseBody{
		Limit:  *v.Limit,
		Offset: *v.Offset,
		Total:  *v.Total,
	}

	return res
}

// marshalIngestviewsSIPWorkflowViewToSIPWorkflowResponseBody builds a value of
// type *SIPWorkflowResponseBody from a value of type
// *ingestviews.SIPWorkflowView.
func marshalIngestviewsSIPWorkflowViewToSIPWorkflowResponseBody(v *ingestviews.SIPWorkflowView) *SIPWorkflowResponseBody {
	if v == nil {
		return nil
	}
	res := &SIPWorkflowResponseBody{
		UUID:        *v.UUID,
		TemporalID:  *v.TemporalID,
		Type:        *v.Type,
		Status:      *v.Status,
		StartedAt:   *v.StartedAt,
		CompletedAt: v.CompletedAt,
		SipUUID:     *v.SipUUID,
	}
	if v.Tasks != nil {
		res.Tasks = make([]*SIPTaskResponseBody, len(v.Tasks))
		for i, val := range v.Tasks {
			if val == nil {
				res.Tasks[i] = nil
				continue
			}
			res.Tasks[i] = marshalIngestviewsSIPTaskViewToSIPTaskResponseBody(val)
		}
	}

	return res
}

// marshalIngestviewsSIPTaskViewToSIPTaskResponseBody builds a value of type
// *SIPTaskResponseBody from a value of type *ingestviews.SIPTaskView.
func marshalIngestviewsSIPTaskViewToSIPTaskResponseBody(v *ingestviews.SIPTaskView) *SIPTaskResponseBody {
	if v == nil {
		return nil
	}
	res := &SIPTaskResponseBody{
		UUID:         *v.UUID,
		Name:         *v.Name,
		Status:       *v.Status,
		StartedAt:    *v.StartedAt,
		CompletedAt:  v.CompletedAt,
		Note:         v.Note,
		WorkflowUUID: *v.WorkflowUUID,
	}

	return res
}

// marshalIngestviewsUserViewToUserResponseBody builds a value of type
// *UserResponseBody from a value of type *ingestviews.UserView.
func marshalIngestviewsUserViewToUserResponseBody(v *ingestviews.UserView) *UserResponseBody {
	res := &UserResponseBody{
		UUID:      *v.UUID,
		Email:     *v.Email,
		Name:      *v.Name,
		CreatedAt: *v.CreatedAt,
	}

	return res
}

// marshalIngestviewsSIPSourceObjectViewToSIPSourceObjectResponseBody builds a
// value of type *SIPSourceObjectResponseBody from a value of type
// *ingestviews.SIPSourceObjectView.
func marshalIngestviewsSIPSourceObjectViewToSIPSourceObjectResponseBody(v *ingestviews.SIPSourceObjectView) *SIPSourceObjectResponseBody {
	res := &SIPSourceObjectResponseBody{
		Key:     *v.Key,
		ModTime: v.ModTime,
		Size:    v.Size,
		IsDir:   *v.IsDir,
	}

	return res
}

// marshalIngestviewsBatchViewToBatchResponseBody builds a value of type
// *BatchResponseBody from a value of type *ingestviews.BatchView.
func marshalIngestviewsBatchViewToBatchResponseBody(v *ingestviews.BatchView) *BatchResponseBody {
	res := &BatchResponseBody{
		UUID:          *v.UUID,
		Identifier:    *v.Identifier,
		SipsCount:     *v.SipsCount,
		Status:        *v.Status,
		CreatedAt:     *v.CreatedAt,
		StartedAt:     v.StartedAt,
		CompletedAt:   v.CompletedAt,
		UploaderUUID:  v.UploaderUUID,
		UploaderEmail: v.UploaderEmail,
		UploaderName:  v.UploaderName,
	}

	return res
}
