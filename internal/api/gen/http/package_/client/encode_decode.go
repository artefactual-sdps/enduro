...
// set to call the "package" service "download" endpoint
func (c *Client) BuildDownloadRequest(ctx context.Context, v interface{}) (*http.Request, error) {
	var (
		id uint
	)
	{
		p, ok := v.(*package_.DownloadPayload)
		if !ok {
			return nil, goahttp.ErrInvalidType("package", "download", "*package_.DownloadPayload", v)
		}
		id = p.ID
	}
	u := &url.URL{Scheme: c.scheme, Host: c.host, Path: DownloadPackagePath(id)}
	req, err := http.NewRequest("GET", u.String(), nil)
	if err != nil {
		return nil, goahttp.ErrInvalidURL("package", "download", u.String(), err)
	}
	if ctx != nil {
		req = req.WithContext(ctx)
	}
...
// package download endpoint. restoreBody controls whether the response body
// should be restored after having been read.
...
func DecodeDownloadResponse(decoder func(*http.Response) goahttp.Decoder, restoreBody bool) func(*http.Response) (interface{}, error) {
	return func(resp *http.Response) (interface{}, error) {
		...
		case http.StatusOK:
			...
			err = decoder(resp).Decode(&body)
			if err != nil {
				return nil, goahttp.ErrDecodingError("package", "download", err)
			}
			return body, nil
		case http.StatusNotFound:
			var (
			...
			if err != nil {
				return nil, goahttp.ErrValidationError("package", "download", err)
			}
			return nil, NewDownloadNotFound(&body)
		default:
			body, _ := ioutil.ReadAll(resp.Body)
			return nil, goahttp.ErrInvalidResponse("package", "download", resp.StatusCode, string(body))
		}
	}
}
