...
func NewEndpoints(s Service) *Endpoints {
	...
	return &Endpoints{
		...
		ListSourceItems:    NewListSourceItemsEndpoint(s, a.JWTAuth),
	}
}
...
func (e *Endpoints) Use(m func(goa.Endpoint) goa.Endpoint) {
	...
	e.ListSourceItems = m(e.ListSourceItems)
}
...
// NewListSourceItemsEndpoint returns an endpoint function that calls the
// method "list_source_items" of service "ingest".
func NewListSourceItemsEndpoint(s Service, authJWTFn security.AuthJWTFunc) goa.Endpoint {
	return func(ctx context.Context, req any) (any, error) {
		p := req.(*ListSourceItemsPayload)
		var err error
		sc := security.JWTScheme{
			Name:           "jwt",
			Scopes:         []string{"ingest:sips:create", "ingest:sips:download", "ingest:sips:list", "ingest:sips:read", "ingest:sips:review", "ingest:sips:upload", "ingest:sips:workflows:list", "ingest:sources:items:list", "ingest:users:list", "storage:aips:create", "storage:aips:deletion:request", "storage:aips:deletion:review", "storage:aips:download", "storage:aips:list", "storage:aips:move", "storage:aips:read", "storage:aips:review", "storage:aips:submit", "storage:aips:workflows:list", "storage:locations:aips:list", "storage:locations:create", "storage:locations:list", "storage:locations:read"},
			RequiredScopes: []string{"ingest:sources:items:list"},
		}
		var token string
		if p.Token != nil {
			token = *p.Token
		...
		res, err := s.ListSourceItems(ctx, p)
		if err != nil {
			return nil, err
		}
		vres := NewViewedSourceItems(res, "default")
		return vres, nil
	}
}
