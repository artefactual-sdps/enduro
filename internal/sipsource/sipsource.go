...
// ByDesc sets the sort order to descending (Z-A).
func (s *Sort) ByDesc() *Sort {
	s.desc = true
	return s
}

// ByAsc sets the sort order to ascending (A-Z).
func (s *Sort) ByAsc() *Sort {
	s.desc = false
	return s
}

// Compare two objects based on the Sort configuration. Compare returns -1 if
// `a` sorts before `b`, 0 if `a` and `b` are equal in sort order, and 1
// if `a` sorts after `b`. If Sort is nil or has no attribute set, Compare
// returns 0.
