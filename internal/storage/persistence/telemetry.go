...
func (w *wrapper) AIPDBID(ctx context.Context, aipID uuid.UUID) (int, error) {
	ctx, span := w.tracer.Start(ctx, "AIPDBID")
	defer span.End()

	r, err := w.wrapped.AIPDBID(ctx, aipID)
	if err != nil {
		telemetry.RecordError(span, err)
		return r, updateError(err, "AIPDBID")
	}
