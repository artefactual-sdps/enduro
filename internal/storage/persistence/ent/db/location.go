	...
	for i := range columns {
		...
		case location.FieldSource:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				_m.Source = enums.LocationSource(value.String)
			}
		case location.FieldPurpose:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field purpose", values[i])
			} else if value.Valid {
				_m.Purpose = enums.LocationPurpose(value.String)
			}
		case location.FieldUUID:
...
func (_m *Location) QueryAips() *AIPQuery {
	return NewLocationClient(_m.config).QueryAips(_m)
}
