...
func (wq *WorkflowQuery) QueryAip() *AIPQuery {
	query := (&AIPClient{config: wq.config}).Query()
...
func (wq *WorkflowQuery) WithAip(opts ...func(*AIPQuery)) *WorkflowQuery {
	query := (&AIPClient{config: wq.config}).Query()
...
func (wq *WorkflowQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Workflow, error) {
	...
	if query := wq.withAip; query != nil {
		if err := wq.loadAip(ctx, query, nodes, nil,
...
func (wq *WorkflowQuery) loadAip(ctx context.Context, query *AIPQuery, nodes []*Workflow, init func(*Workflow), assign func(*Workflow, *AIP)) error {
