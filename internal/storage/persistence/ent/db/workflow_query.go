...
func (_q *WorkflowQuery) QueryAip() *AIPQuery {
...
func (_q *WorkflowQuery) QueryTasks() *TaskQuery {
...
func (_q *WorkflowQuery) QueryDeletionRequest() *DeletionRequestQuery {
...
func (_q *WorkflowQuery) Clone() *WorkflowQuery {
	...
	return &WorkflowQuery{
		...
		withDeletionRequest: _q.withDeletionRequest.Clone(),
...
func (_q *WorkflowQuery) WithAip(opts ...func(*AIPQuery)) *WorkflowQuery {
	query := (&AIPClient{config: _q.config}).Query()
...
func (_q *WorkflowQuery) WithTasks(opts ...func(*TaskQuery)) *WorkflowQuery {
	query := (&TaskClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *WorkflowQuery) WithDeletionRequest(opts ...func(*DeletionRequestQuery)) *WorkflowQuery {
	query := (&DeletionRequestClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withDeletionRequest = query
	return _q
}
...
func (_q *WorkflowQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Workflow, error) {
	...
	if query := _q.withAip; query != nil {
		if err := _q.loadAip(ctx, query, nodes, nil,
			func(n *Workflow, e *AIP) { n.Edges.Aip = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withDeletionRequest; query != nil {
		if err := _q.loadDeletionRequest(ctx, query, nodes, nil,
...
func (_q *WorkflowQuery) loadAip(ctx context.Context, query *AIPQuery, nodes []*Workflow, init func(*Workflow), assign func(*Workflow, *AIP)) error {
...
func (_q *WorkflowQuery) loadTasks(ctx context.Context, query *TaskQuery, nodes []*Workflow, init func(*Workflow), assign func(*Workflow, *Task)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(task.FieldWorkflowID)
	}
...
func (_q *WorkflowQuery) loadDeletionRequest(ctx context.Context, query *DeletionRequestQuery, nodes []*Workflow, init func(*Workflow), assign func(*Workflow, *DeletionRequest)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Workflow)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (_q *WorkflowQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withAip != nil {
			_spec.Node.AddColumnOnce(workflow.FieldAipID)
		}
	}
