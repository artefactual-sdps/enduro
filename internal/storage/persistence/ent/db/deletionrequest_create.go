...
func (drc *DeletionRequestCreate) check() error {
	...
	if _, ok := drc.mutation.Reviewer(); !ok {
		return &ValidationError{Name: "reviewer", err: errors.New(`db: missing required field "DeletionRequest.reviewer"`)}
	}
	if _, ok := drc.mutation.ReviewerIss(); !ok {
		return &ValidationError{Name: "reviewer_iss", err: errors.New(`db: missing required field "DeletionRequest.reviewer_iss"`)}
	}
	if _, ok := drc.mutation.ReviewerSub(); !ok {
		return &ValidationError{Name: "reviewer_sub", err: errors.New(`db: missing required field "DeletionRequest.reviewer_sub"`)}
	}
