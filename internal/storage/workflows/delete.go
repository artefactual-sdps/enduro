...
func (w *StorageDeleteWorkflow) review(
	...
	// Wait for user dec signal.
	var dec storage.DeletionDecisionSignal
	open := temporalsdk_workflow.GetSignalChannel(ctx, storage.DeletionDecisionSignalName).Receive(ctx, &dec)
	if !open {
		return nil, fmt.Errorf("deletion review signal channel closed")
	}

	logger.Info("Received AIP deletion workflow decision", "signal", dec)
