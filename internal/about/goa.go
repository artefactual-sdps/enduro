...
func (s *Service) About(context.Context, *goaabout.AboutPayload) (*goaabout.EnduroAbout, error) {
	...
	// Add enabled child workflows to the response.
	if len(s.cwfConfigs) > 0 {
		// Preallocate a slice with a capacity equal to the number of configured
		// child workflows to avoid multiple allocations, but the actual length
		// will depend on how many are enabled.
		...
		for _, cfg := range s.cwfConfigs {
			...
			res.ChildWorkflows = append(
				res.ChildWorkflows,
				&goaabout.EnduroChildworkflow{
