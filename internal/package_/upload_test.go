...
func TestUpload(t *testing.T) {
	...
	t.Run("Writes only the first multipart of the request to the bucket", func(t *testing.T) {
		...
		err := svc.Goa().Upload(ctx, &goapackage.UploadPayload{ContentType: "multipart/form-data; boundary=foobar"}, r)
		assert.NilError(t, err)
	...
	t.Run("Returns invalid_media_type if media type cannot be parsed", func(t *testing.T) {
		...
		err := svc.Goa().Upload(ctx, &goapackage.UploadPayload{ContentType: "invalid type"}, r)
