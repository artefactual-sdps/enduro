...
// workflowState is shared state that is passed down to activities. It can be
...
// PIPInfo is data about the PIP, if one is created. It is populated by
// BundleActivity.
type PIPInfo struct {
	Path string
}

// AIPInfo is data about the AIP, if one is created.
type AIPInfo struct {
	// ID is the AIP UUID.
	//
...
func (w *ProcessingWorkflow) sessionCleanup(ctx temporalsdk_workflow.Context, state *workflowState) {
	if state.status != enums.WorkflowStatusDone {
		if err := w.sendToFailedBucket(ctx, state.sip.SendToFailed, state.req.Key); err != nil {
			w.logger.Error(
...
func (w *ProcessingWorkflow) SessionHandler(
	...
	// Download.
	{
		...
		state.sip.SendToFailed.Path = downloadResult.Path
		state.sip.SendToFailed.ActivityName = activities.SendToFailedSIPsName
	}
...
func (w *ProcessingWorkflow) transferA3m(
	...
	// Bundle PIP as an Archivematica standard transfer.
	{
		...
		state.sip.Type = enums.SIPTypeArchivematicaStandardTransfer
	...
	state.sip.SendToFailed.Path = state.pip.Path
	state.sip.SendToFailed.ActivityName = activities.SendToFailedPIPsName
	state.sip.SendToFailed.NeedsZipping = true
...
func (w *ProcessingWorkflow) transferAM(
	...
	if state.sip.Type != enums.SIPTypeBagIt {
		...
		state.sip.Type = enums.SIPTypeBagIt
	}
	...
	state.sip.SendToFailed.Path = state.sip.Path
	state.sip.SendToFailed.ActivityName = activities.SendToFailedPIPsName
	state.sip.SendToFailed.NeedsZipping = true
	...
	if w.cfg.AM.ZipPIP {
		...
		state.sip.SendToFailed.Path = zipResult.Path
		state.sip.SendToFailed.NeedsZipping = false
