...
func (s *EventServiceInMemImpl) PublishEvent(ctx context.Context, event any) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *EventServiceInMemImpl) Subscribe(ctx context.Context) (Subscription, error) {
	sub := &SubscriptionInMemImpl{
		service: s,
		c:       make(chan any, EventBufferSize),
		id:      uuid.New(),
	}
...
func (s *SubscriptionInMemImpl) C() <-chan any {
	return s.c
}
