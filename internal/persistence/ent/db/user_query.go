...
type UserQuery struct {
	...
	withUploadedSips    *SIPQuery
	withUploadedBatches *BatchQuery
...
// QueryUploadedSips chains the current query on the "uploaded_sips" edge.
func (_q *UserQuery) QueryUploadedSips() *SIPQuery {
...
// QueryUploadedBatches chains the current query on the "uploaded_batches" edge.
func (_q *UserQuery) QueryUploadedBatches() *BatchQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withUploadedSips:    _q.withUploadedSips.Clone(),
		withUploadedBatches: _q.withUploadedBatches.Clone(),
...
// WithUploadedSips tells the query-builder to eager-load the nodes that are connected to
// the "uploaded_sips" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithUploadedSips(opts ...func(*SIPQuery)) *UserQuery {
	query := (&SIPClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withUploadedSips = query
	return _q
}

// WithUploadedBatches tells the query-builder to eager-load the nodes that are connected to
// the "uploaded_batches" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithUploadedBatches(opts ...func(*BatchQuery)) *UserQuery {
	query := (&BatchClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withUploadedBatches = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		_spec       = _q.querySpec()
		loadedTypes = [2]bool{
			_q.withUploadedSips != nil,
			_q.withUploadedBatches != nil,
		}
	)
	...
	if query := _q.withUploadedSips; query != nil {
		if err := _q.loadUploadedSips(ctx, query, nodes,
			func(n *User) { n.Edges.UploadedSips = []*SIP{} },
			func(n *User, e *SIP) { n.Edges.UploadedSips = append(n.Edges.UploadedSips, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withUploadedBatches; query != nil {
		if err := _q.loadUploadedBatches(ctx, query, nodes,
			func(n *User) { n.Edges.UploadedBatches = []*Batch{} },
			func(n *User, e *Batch) { n.Edges.UploadedBatches = append(n.Edges.UploadedBatches, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadUploadedSips(ctx context.Context, query *SIPQuery, nodes []*User, init func(*User), assign func(*User, *SIP)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(sip.FieldUploaderID)
	}
	query.Where(predicate.SIP(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.UploadedSipsColumn), fks...))
	}))
...
func (_q *UserQuery) loadUploadedBatches(ctx context.Context, query *BatchQuery, nodes []*User, init func(*User), assign func(*User, *Batch)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(batch.FieldUploaderID)
	}
	query.Where(predicate.Batch(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.UploadedBatchesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
