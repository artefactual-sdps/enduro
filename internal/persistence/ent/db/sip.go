...
type SIP struct {
	...
	// ChecksumValue holds the value of the "checksum_value" field.
...
func (*SIP) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case sip.FieldName, sip.FieldStatus, sip.FieldFailedAs, sip.FieldFailedKey, sip.FieldChecksumAlgorithm, sip.FieldChecksumValue:
...
func (_m *SIP) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case sip.FieldChecksumValue:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field checksum_value", values[i])
			} else if value.Valid {
				_m.ChecksumValue = value.String
			}
		default:
...
func (_m *SIP) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("checksum_value=")
	builder.WriteString(_m.ChecksumValue)
	builder.WriteByte(')')
