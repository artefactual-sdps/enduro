...
func (wq *WorkflowQuery) QuerySip() *SIPQuery {
	query := (&SIPClient{config: wq.config}).Query()
...
func (wq *WorkflowQuery) WithSip(opts ...func(*SIPQuery)) *WorkflowQuery {
	query := (&SIPClient{config: wq.config}).Query()
...
//		GroupBy(workflow.FieldWorkflowID).
//		Aggregate(db.Count()).
//		Scan(ctx, &v)
...
//
//	client.Workflow.Query().
//		Select(workflow.FieldWorkflowID).
//		Scan(ctx, &v)
...
func (wq *WorkflowQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Workflow, error) {
	...
	if query := wq.withSip; query != nil {
		if err := wq.loadSip(ctx, query, nodes, nil,
...
func (wq *WorkflowQuery) loadSip(ctx context.Context, query *SIPQuery, nodes []*Workflow, init func(*Workflow), assign func(*Workflow, *SIP)) error {
