...
describe("Pager", () => {
  it("renders the correct number of pages when totalPages is less than maxVisiblePages", () => {
    ...
    expect(pageItems.length).toBe(9); // 5 pages + First + Last.
    expect(wrapper.findAll(".page-item:not(.disabled)").length).toBe(7); // Active pages
  });
  ...
  it("disables the First and Previous buttons on the first page", () => {
    ...
    const firstButton = wrapper.find("li.page-item:first-child");
    const prevButton = wrapper.find("li.page-item:nth-child(2)");

    expect(firstButton.classes()).toContain("disabled");
  ...
  it("disables the Last and Next buttons on the last page", () => {
    ...
    const nextButton = wrapper.find("li.page-item:nth-last-child(2)");
    const lastButton = wrapper.find("li.page-item:last-child");

    expect(nextButton.classes()).toContain("disabled");
    expect(lastButton.classes()).toContain("disabled");
  });
